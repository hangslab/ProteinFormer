...
parser.add_argument('--save_root', default='debug', help="name your save_root")
...
train_file = "filenames/setall/labels_train.txt"
valid_file = "filenames/setall/labels_val.txt"
test_file = "filenames/setall/labels_test.txt"
...
def train(args=None):
    print('start_epoch', start_epoch)
    if start_epoch != 0:
        save_NAME = '%d_%s_%d.pth' % (start_epoch, 'acc', 0)
        save_PATH = os.path.join(args.save_root, save_NAME)
        torch.save({'start_epoch': start_epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, save_PATH)
        _ = evaluate(args, start_epoch, eval_loader)
