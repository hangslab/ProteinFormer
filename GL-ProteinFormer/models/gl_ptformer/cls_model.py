...
class CLSModel(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm):
        ...
        self.head = ResnetDecoder(2048, 13)
        # self.head = None
    def forward(self, x):
...
class PTFormer_H(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm):
        super(PTFormer_H, self).__init__()
        # self.backbone = swin_base(out_indices=[3], in_chans=4)
        self.backbone = ResNet(Bottleneck, [3, 4, 6, 3], in_c=4, num_classes=13)
        # self.head = ResnetDecoder(1024, 13)
        ...
        embed_dim = 1024
        self.patch = PatchEmbed(patch_size=4, in_chans=3840, embed_dim=embed_dim)
        ...
        self.decoder = Transformer(dim=embed_dim, depth=3, heads=8, dim_head=64, mlp_dim=embed_dim)
        self.to_latent = nn.Identity()
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, 13)
...
class GL_PTFormer(nn.Module):
    def __init__(self, n_classes, norm_layer=nn.LayerNorm):
        super(GL_PTFormer, self).__init__()
    ...
    def forward(self, x):
        ...
        _, fc, fh, fw = feat.shape
        # print('feat.shape', feat.shape)
        token = feat.reshape(B, fc, fh, fw).permute(0, 2, 3, 1).flatten(1, 2)
        # print('token.shape', token.shape)
