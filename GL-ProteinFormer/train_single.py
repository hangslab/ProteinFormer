...
train_annotation_file = 'filenames/single_cls/train.txt'
val_annotation_file = 'filenames/single_cls/val.txt'
test_annotation_file = 'filenames/single_cls/test.txt'

data_root = 'data_root'
train_dataset = HpaDataset(split='train', root=data_root, annotation=train_annotation_file)
print("training data count: %d" % len(train_dataset))
valid_dataset = HpaDataset(split='test', root=data_root, annotation=val_annotation_file)
print("testing data count: %d" % len(valid_dataset))
test_dataset = HpaDataset(split='test', root=data_root, annotation=train_annotation_file)
...
model = nn.DataParallel(GL_PTFormer(n_classes=7), device_ids=gpus)


# model = get_resnet50(3, n_classes=7)
# model = nn.DataParallel(CLSModel(), device_ids=gpus)
# model = nn.DataParallel(CLSModel_Pre(n_classes=7), device_ids=gpus)
# model = nn.DataParallel(CLSModel(n_classes=7), device_ids=gpus)
...
# model = nn.DataParallel(swin_base(in_chans=3, num_classes=7), device_ids=gpus)

# model = nn.DataParallel(CLSModel(), device_ids=gpus)
# model = nn.DataParallel(STModel(), device_ids=gpus)
# model = nn.DataParallel(MultiScaleSTModel(), device_ids=gpus)
